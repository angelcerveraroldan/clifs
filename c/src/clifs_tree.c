...
void init_node(struct cfs_node *node, metadata_t meta, node_kind_t node_k, const char *name) {
    ...
    if (node_k == CFS_DIR)
        init_children(&node->data.dir_children);
    else
        cstr_set(&node->data.file_content, "\0");
}
...
int adopt_child(struct cfs_node *r, struct cfs_node *c) {
    ...
    // Name already in use
    if (find_child_with_name(&r->data.dir_children, c->name.data) != -1) return -EEXIST;
...
void free_node(struct cfs_node *node) {
    ...
    if (node->node_k == CFS_DIR)
        free_all_children(&node->data.dir_children);
    else
        free_cstr(&node->data.file_content);
